type StrokeIcon = 'mail' | 'phone' | 'pin' | 'external'
type BrandIcon = 'github' | 'linkedin'
export type IconName = StrokeIcon | BrandIcon

const strokePaths: Record<StrokeIcon, JSX.Element> = {
  mail: (
    <>
...
const isBrand = (name: IconName): name is BrandIcon => name in brandPaths

/** inline SVG so icons stay crisp at any DPI and inherit currentColor */
const Icon = ({ name, size = 20, className = '' }: { name: IconName, size?: number, className?: string }) => {
  if (isBrand(name)) {
    return (
